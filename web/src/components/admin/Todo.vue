...
<div class="cardTodo" @contextmenu="handler()">
    <div class="detailTodo" @click.left="openDetailCard" @click.right="openQuickEdit">
        ...
        <div class="list-card-labels">
            <span class="card-label card-label-default mod-card-front" @click="handleShowLable" :class="[{'labelShow' :labelShow}, 'card-label-'+label.color]" v-for="label in card.labels" :key="label.id" title="LMS Thầy thiện">
        ...
        <div class="badges">
            <span class="js-badges">
                <div class="badge js-due-date-badge mod-due-date" :class="[{'is-due-complete':card.status === 3},{'is-due-die': card.status===1},{'is-due-near': card.status===2}]" title="This card is complete." v-if="card.deadline!=null">
                    <span class="badge-icon icon-sm icon-clock badge-due-icon"> <i class="el-icon-alarm-clock badge-due-icon"></i></span>
